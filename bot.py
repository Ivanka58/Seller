...
def button_actions(call):
    ...
    if action == "block":
        # Сразу блокируем пользователя без дополнительного выбора
        bot.send_message(GROUP_ID, f"Пользователь @{bot.get_chat(chat_id).username} заблокирован по стандартной причине.")
        bot.send_message(int(chat_id), f"Вы заблокированы администрацией по стандартной причине.")
    elif action == "warn":
        # Сразу выдаём предупреждение без дополнительного выбора
        current_warnings = warnings_db.get(chat_id, 0)
        new_warnings = current_warnings + 1
        warnings_db[chat_id] = new_warnings
        warning_level = f"{new_warnings}/3"
        bot.send_message(GROUP_ID, f"Пользователю @{bot.get_chat(chat_id).username} выдано предупреждение {warning_level} по стандартной причине.")
        bot.send_message(int(chat_id), f"Вам выдано предупреждение {warning_level} по стандартной причине. Не нарушайте правила.")
        if new_warnings >= 3:
            bot.send_message(GROUP_ID, f"Пользователь @{bot.get_chat(chat_id).username} получил последнее предупреждение и заблокирован.")
            bot.send_message(int(chat_id), f"Вы получили предупреждение 3/3 по стандартной причине. Вы заблокированы.")
