...
# --- ФУНКЦИОНАЛЬНОСТЬ БЛОИРОВКИ И ПРЕДУПРЕЖДЕНИЙ ---
def block_user(chat_id):
    warnings_db[chat_id] = 3  # Максимальное количество предупреждений
...
Добавлены функции блокировки и выдачи предупреждений.
Обработчики кнопок (block и warn) реализованы и корректно функционируют.


Вторая итерация: добавим кнопки "Отмена" и полноценную обработку уведомлений.

Шаги:

Добавим кнопки "Отмена" в группу сотрудников.
Расширим функционал уведомлений, чтобы поддерживать удобное взаимодействие.
